"""
Train a PGDD-Net model on a custom dataset.
Models and datasets download automatically from the latest PGDD-Net release.
...
ROOT = FILE.parents[0]  # PGDD-Net root directory
...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    if pretrained:
        ...
        LOGGER.info(f'Transferred {len(csd)}/{len(model.state_dict())} items from ')  # report
    else:
    ...
    if RANK in [-1, 0]:
        val_loader = create_dataloader(val_path, imgsz, batch_size // WORLD_SIZE * 2, gs, single_cls,
                                       ...
                                       rect=False, rank=-1, workers=workers * 2, pad=2,
...
def parse_opt(known=False):
    ...
    parser.add_argument('--cfg', type=str, default='models/PGDD-Net.yaml', help='model.yaml path')
    parser.add_argument('--data', type=str, default='/home/szz/projects/yoloair04/data/al.yaml', help='dataset.yaml path')
...
def main(opt, callbacks=Callbacks()):
    ...
    if LOCAL_RANK != -1:
        msg = 'is not compatible with PGDD-Net Multi-GPU DDP training'
...
def run(**kwargs):
    # Usage: import train; train.run(data='coco128.yaml', imgsz=320, weights='PGDD-Net.pt')
    opt = parse_opt(True)
